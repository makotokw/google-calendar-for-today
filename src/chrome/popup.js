(function($){
  $(document).ready(function() {
    ...
    $('#calendar,#logo')
      ...
      .click(function(e){
        var calendar = app.getCalendarUrl();
    ...
    function update() {
      ...
      for (var index = 0, showDays = (!isNaN(displayDayCount)) ? displayDayCount : 2; index < showDays; index++) {
        ...
        var eventCount = 0,
            displayEventWithoutAllDayCount = 0,
            displayEventCount = 0,
            nextEventIndex = -1;

        if (events.length > 0) {
          events.sort(app.appointmentCompare);
          var $list = $('<div/>').addClass('event_list');
          if (offset === 0) $list.addClass('today');
          var $nowListItem = $('<div/>'),
              isAppendedNow = false;

          $nowListItem
            .addClass('event now_marker')
            .attr({title:now.toLocaleTimeString()})
          ...
          for (var i=0, len=events.length; i<len; i++) {
            ...
            var $li = $('<div/>').addClass('event');
                $detail = $('<div/>').addClass('event_details');
            $li.append($('<div/>').addClass('feed_color').css({'background-color':color}));
            if (offset === 0) { // Today
              if (!event.allDay) {
            ...
            $detail.append($('<a/>').attr({href:event.link,target:'_blank'}).addClass('event_title').html(event.title));
            $detail.append($('<span/>').addClass((event.allDay) ? 'event_date event_date_allday' : 'event_date').html(time));
            if (location && location.length > 0) {
              $detail.append($('<span/>').addClass('event_location').html(location));
            }
            if (nextEventIndex==i) $detail.append($('<span/>').addClass('event_timer').html(remain));
            $li.append($detail);
